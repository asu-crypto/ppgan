...
def train_half(gen, disc, args):
    ...
    bceloss = crypten.nn.BCELoss()#lambda x,y: (x-y).square().mean()#crypten.nn.BCELoss()
    ...
    while iteration < args.iteration_number:
        for img, _ in dataloader:
            ...
            # real image
            crypt_img = crypten.cryptensor(img).to(args.device) #, gpu is slower
