...
def train_hybrid(gen, disc, pub, args):
        ...
        for img, _ in dataloader:
            # try:
                ...
                if iteration % 10 == 9 or True:
                    ...
                    private_grad_enc = crypten.cryptensor(private_grad*100)
                    # print(private_grad_enc.get_plain_text().abs().mean())
                    # backward pass such gradient
                    fake_intermediate_output_enc.backward(grad_input=private_grad_enc/100, retain_graph=True)
