...
class AutogradLeakyReLU(AutogradFunction):
    @staticmethod
    def forward(ctx, input, slope=0.01):
        mask = input.gt(0.0)
        slope = crypten.cryptensor(slope,device=input.device)
        ctx.save_multiple_for_backward([mask, slope])
        
        return input.mul(mask + slope - mask.mul(slope))
    
    @staticmethod
    def backward(ctx, grad_output):
        mask, slope = ctx.saved_tensors
        return grad_output.mul(mask + slope - slope*mask)
