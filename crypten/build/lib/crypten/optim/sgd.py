        ...
        with crypten.no_grad():
            ...
            for group in self.param_groups:
                ...
                for p in group["params"]:
                    ...
                    if self.square_threshold is not None:
                        lt_minus1 = p.grad.lt(-1)
                        gt_plus1 = p.grad.gt(1)
                        d_p = -lt_minus1+gt_plus1+p.grad.mul(1-lt_minus1-gt_plus1)#p.grad.mul(p.grad.square().lt(self.square_threshold))+p.grad.lt(-1)+p.grad.gt(1)
                    else:
                        d_p = p.grad
