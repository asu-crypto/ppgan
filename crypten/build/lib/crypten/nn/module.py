...
class Gemm(Module):
    ...
    def forward(self, x):
        a, b, c = tuple(x)
        # print(a.device, b.device,c.device,self.alpha.device)
        # assert a.device==b.device, f"{a.device} {b.device} {a.shape} {b.shape}"
        if self.trans_a:
            a = a.t()
        if self.trans_b:
            b = b.t()
...
class LeakyReLU(Module):
    ...
    :math:`\text{LeakyReLU}(x)= [(x>0) + (1-(x>0))*slope]*x`
    """
    ...
    def from_onnx(attributes=None):
        ...
        return LeakyReLU(negative_slope=attributes.get("alpha"))
