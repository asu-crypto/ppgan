...
def feat_mse(img_batch, ref_batch, model):
    feats_rec = model(ref_batch.to('cuda'))
    feats_true = model(img_batch.to('cuda'))
    return (feats_rec - feats_true).pow(2).mean().item()#[..., relevant_features.view(-1)]
