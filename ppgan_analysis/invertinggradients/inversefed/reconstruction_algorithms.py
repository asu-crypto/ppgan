...
class GradientReconstructor():
    ...
    def __init__(self, model, loss_fn, mean_std=(0.0, 1.0), config=DEFAULT_CONFIG, num_images=1):
        ...
        # self.loss_fn = torch.nn.BCELoss()#torch.nn.CrossEntropyLoss(reduction='mean')
        self.iDLG = True
    ...
    def _gradient_closure(self, optimizer, x_trial, input_gradient, label):
        ...
        def closure():
            ...
            # print(x_trial.device, label.device)
            loss = self.loss_fn(self.model(x_trial).squeeze(), label)
